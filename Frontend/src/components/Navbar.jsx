    ...
    <nav className="fixed z-50 top-0 left-0 w-full h-20 bg-gray-900 bg-opacity-70 backdrop-blur-md shadow-md flex items-center">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 flex justify-between items-center w-full">
        ...
        <div className="hidden lg:flex lg:space-x-6">
          <Link
            to="/"
            className="text-gray-200 hover:bg-gray-700 px-4 py-2 rounded-md text-lg"
            onClick={closeMenu}
          >
            Home
          </Link>
        ...
        <div className="hidden lg:flex lg:space-x-6">
          {!isAuthenticated ? (
            <>
              <Link
                to="/auth/login"
                className="text-indigo-400 px-6 py-2 font-semibold"
                onClick={closeMenu}
              >
                Login
              </Link>
              <Link
                to="/auth/signup"
                className="text-teal-400 px-6 py-2 font-semibold"
                onClick={closeMenu}
              >
                Signup
              </Link>
            ...
            <button
              ...
              className="text-indigo-400 px-6 py-2 font-semibold flex items-center"
            >
        ...
        <div className="absolute top-20 left-0 w-full bg-gray-900 bg-opacity-90 text-white rounded-lg shadow-lg px-8 py-4 lg:hidden">
          ...
          {!isAuthenticated ? (
            <>
              <Link
                to="/auth/login"
                className="block text-lg px-6 py-2"
                onClick={closeMenu}
              >
                Login
              </Link>
            ...
            <button
              onClick={handleLogout}
              className="block text-lg px-6 py-2 text-red-500"
            >
              <FaSignOutAlt className="mr-2" /> Logout
            </button>
          )}
