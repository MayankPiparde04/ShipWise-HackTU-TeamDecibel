...
const ItemCalculation = () => {
  ...
  const { productName, shape, weight, quantity, price, dimensions } = state.itemData;
  ...
  const renderDimensions = () => {
    ...
    switch (shape.toLowerCase()) {
      case "cube":
        return side && <p><span className="font-semibold">Side:</span> {side}</p>;

      case "cuboid":
        return (
          <>
            {length && <p><span className="font-semibold">Length:</span> {length}</p>}
            {breadth && <p><span className="font-semibold">Breadth:</span> {breadth}</p>}
            {height && <p><span className="font-semibold">Height:</span> {height}</p>}
          </>
        );
      ...
      case "cylinder":
        return (
          <>
            {radius && <p><span className="font-semibold">Radius:</span> {radius}</p>}
            {height && <p><span className="font-semibold">Height:</span> {height}</p>}
          </>
        );

      case "sphere":
        return radius && <p><span className="font-semibold">Radius:</span> {radius}</p>;
  ...
  const handleSubmit = async () => {
    ...
    const data = {
      product: {
        ...
        breadth: parseFloat(itemData.breadth),
    ...
    <div className="bg-gray-900 text-gray-200 min-h-screen p-6 pt-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4 p-6 bg-gray-800 rounded-lg">
          ...
          <p><span className="font-semibold">Shape:</span> {shape}</p>
          <p><span className="font-semibold">Weight:</span> {weight}</p>
          <p><span className="font-semibold">Quantity Available:</span> {quantity}</p>
          <p><span className="font-semibold">Per Item Price:</span> {price}</p>
          <p><span className="font-semibold">Total Price:</span> {price * selectedQuantity}</p>
          {renderDimensions()}
          <div>
            <label className="block text-sm font-semibold">Select Quantity</label>
            <input
              type="number"
          ...
          {results && (
            <div className="w-2/3 flex flex-wrap gap-4">
              ...
              <div className="mt-4 w-full">
                <p className="font-medium text-green-400">Remaining Quantity: {results.remaining_quantity}</p>
                <p className="font-medium text-green-400">Success: {results.success ? "Yes" : "No"}</p>
              </div>
            </div>
          )}
        </div>
